...
export const Item = ({ prod }) => {
    ...
    const agregarCarrito = `mb-4 bg-blue-700 sm:w-[75%] w-[55%] text-[12px] text-semibold m-auto px-4 py-2 rounded-md hover:bg-blue-400 focus:outline ${count == 0 ? 'block' : 'hidden'}`;
    const cargadoCarrito = `text-white container m-auto mb-6 mt-3 ${count === 0 ? 'hidden' : 'block'}`
        ...
        <>
            <section className="bg-[#313131] w-40 flex flex-col justify-between m-8 rounded sm:w-52 border-[1px] ">
                ...
                <div className="flex flex-col m-2">
                    ...
                    <button className={agregarCarrito} onClick={handleIncrement}>
                        Agregar al carrito
                    </button>
                    <section className={cargadoCarrito}>
                        <div className='flex items-center justify-center space-x-4'>
                            <button className='bg-blue-700 hover:bg-red-700 text-while px-4 rounded' onClick={handleDecrement}>
                                -
                            </button>
                            <span className='text-xl font-bold'>
                                {count}
                            </span>
                            <button className='bg-blue-700 hover:bg-green-700 text-while px-4 rounded' onClick={handleIncrement}>
                                +
                            </button>
